...
public class Boot4RedissonAutoConfiguration {
    ...
    public RedissonAutoConfigurationCustomizer redissonAutoConfigurationCustomizers() {
        return config -> {
            // 序列化模式
            TypedJsonJackson3Codec jsonCodec = new TypedJsonJackson3Codec(Object.class, objectMapper());
            JsonJackson3Codec jackson3Codec = new JsonJackson3Codec(objectMapper());
            // 组合序列化 key 使用 String 内容使用通用 json 格式
            config.setCodec(new CompositeCodec(StringCodec.INSTANCE, jackson3Codec, jackson3Codec));
