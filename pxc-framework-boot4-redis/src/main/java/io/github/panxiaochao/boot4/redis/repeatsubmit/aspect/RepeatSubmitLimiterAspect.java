package io.github.panxiaochao.boot4.redis.repeatsubmit.aspect;
...
import io.github.panxiaochao.boot4.redis.repeatsubmit.annotation.RepeatSubmitLimiter;
...
import io.github.panxiaochao.boot4.utils.RequestUtil;
...
public class RepeatSubmitLimiterAspect {
    ...
    public RepeatSubmitLimiterAspect() {
        LOGGER.info("配置[Redis -> RepeatSubmitLimiterAspect]成功！");
    }
    ...
    public void before(JoinPoint joinPoint, RepeatSubmitLimiter repeatSubmitLimiter) {
        ...
        if (timeUnit.toMillis(interval) < 1000) {
            throw new FrameworkRuntimeException(RepeatSubmitLimiterErrorEnum.REPEAT_SUBMIT_LIMITER_TIME_ERROR);
        }
        // 获取限重复提交KEY
        ...
        if (RedissonUtil.setIfAbsent(repeatSubmitLimiterKey, "", Duration.ofMillis(interval))) {
            ...
            throw new FrameworkRuntimeException(RepeatSubmitLimiterErrorEnum.REPEAT_SUBMIT_LIMITER_ERROR, message);
        }
    }
